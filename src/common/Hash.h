...
template<>
struct std::hash<common::HashedString>
{
    std::size_t operator()(common::HashedString const& s) const noexcept
    {
        return s.hash;
    }
};

inline std::ostream& operator<<(std::ostream& os, const common::HashedString& h) {
    return os << h.str;
}
